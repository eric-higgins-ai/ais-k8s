...
func main() {
	...
	mgrOptions := ctrl.Options{
		...
		// Previously the cache was disabled entirely for pods (DisableFor),
		// but now that the informer is label-restricted we can safely cache
		// AIS pods and read them from the cache.
		Cache: cache.Options{
			ByObject: map[client.Object]cache.ByObject{
				&corev1.Pod{}: {
					Label: labels.SelectorFromSet(labels.Set{cmn.LabelAppPrefixed: cmn.LabelManagedByValue}),
				},
			},
		},
		Scheme:                 scheme,
