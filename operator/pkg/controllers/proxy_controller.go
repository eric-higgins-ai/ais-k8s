...
func (r *AIStoreReconciler) handleProxyState(ctx context.Context, ais *aisv1.AIStore) (result ctrl.Result, err error) {
	...
	if !isStatusCurrent(ss) {
		logger.V(1).Info("Proxy StatefulSet status is stale, re-queueing")
	...
	// Apply scaling (blocked by rollout in progress, unless the rollout is
	// blocked by unschedulable pods and the user is scaling down)
...
func (r *AIStoreReconciler) hasUnschedulableProxyPods(ctx context.Context, ais *aisv1.AIStore) bool {
